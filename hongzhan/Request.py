...
class curl_request:
    ...
    def __init__(self,url,verbose=False,action='get'):
        ...
        self.c.setopt(pycurl.TIMEOUT, 9)
...
#        c.setopt(pycurl.HEADERFUNCTION, d.body_callback)
    ...
    def set_header(self,array):
		self.c.setopt(pycurl.HTTPHEADER,array)
    ...
    def set_post_para(self,para):
        self.c.setopt(pycurl.POST,1)
        self.c.setopt(pycurl.POSTFIELDS,  para)
    ...
    def get_location(self):
		content = self.r.split('\n')
		re = None
		for node in content:
			if node.startswith('Location:'):
				re = node.replace('Location: ','').strip('\n').strip('\r')
			if node.startswith('location:'):
				re = node.replace('location: ','').strip('\n').strip('\r')
		return re
